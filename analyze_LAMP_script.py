...
def get_all_feat():
    ...
    for ii, name in enumerate(g_samplenames):
        X, y = get_feat(name)
        if ii == 0:
            X_all = X
            y_all = y
...
def calc_f1(X,y,X_cv,y_cv,reg,threshold_arr=g_threshold):
    ...
    best_threshold = threshold_arr[np.argmin(f1_arr)]

    return prec_arr, recall_arr, f1_arr, best_threshold
...
if __name__ == '__main__':
    ...
    for ii in np.arange(k_n_iter): 
        ...
        if g_wind==False:
            ...
            prec, recall, f1, best_thres = calc_f1(X_train,y_train,X_cv,y_cv,best_reg)
            all_prec[ii,:] = prec
    ...
    # Average all arrays
    avg_train = np.mean(all_train[np.isfinite(all_train)],axis=0)
    avg_cv = np.mean(all_cv[np.isfinite(all_cv)],axis=0)
    avg_prec = np.mean(all_prec[np.isfinite(all_prec)],axis=0)
    avg_recall = np.mean(all_recall[np.isfinite(all_recall)],axis=0)
    avg_f1 = np.mean(all_f1[np.isfinite(all_f1)],axis=0)
    avg_theta = np.mean(all_theta,axis=0)
